...
def MPS(fitness, mating_pool_size):
    ...
    selected_to_mate = []
    # First make the probability distribution and save the top boundaries 
    # For each individual's section of the wheel it ranges from 0 to 1
    wheel = []
    total = sum(fitness)
    top = 0
    for i in fitness:
    ...
    # Then add the individual whose distribution the r value is in
